...
const p3 = derive(JSON.parse(readFileSync(new URL("../../examples/social-circle.scenario.json", import.meta.url))));
...
test("Social Circle: 生活流节奏 section rendered, no 赛季/回合 sections", () => {
  const out3 = renderString(tpl, p3);
  assert.match(out3, /## 生活流节奏/);
  assert.doesNotMatch(out3, /## 赛季流程/);
  assert.doesNotMatch(out3, /## 回合流程/);
});
