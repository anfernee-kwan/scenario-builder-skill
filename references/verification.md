...
`tests/unit/skillmd.lint.test.ts` is generated by Scaffold (Tier-A) and runs as part of `npm test`. It will **fail by default** until `src/lib/skillmd.ts` is fully filled.

Tests enforced automatically:

| Test | What it catches |
|------|----------------|
| No `<!-- FILL -->` placeholders | Any unfilled section in rendered output |
| 409 row has concrete description | Generic or dash-only 409 entry |
| 429 row has a rate-limit number | Missing `1500ms` / `2秒` style value |
| 推理建议 has ≥ 20 chars of content | Empty reasoning section |
| 快速开始 has ≥ 2 curl calls | Registration-only quickstart |
| 回合流程 / 赛季流程 has numbered steps | (archetype-conditional) Empty flow section |

A `npm test` run that is green without `src/lib/skillmd.ts` being fully filled is **impossible** — the lint tests will fail first.

---

## Gate 2b — Agent Smoke Gate (run after Gate 2a passes)

`tests/e2e/agent.smoke.test.ts` is generated by Scaffold (Tier-A). The **journey** and **duplicate-action** tests contain `expect.fail(...)` calls that make them **fail by default** until the `FILL:agent-action` zones are replaced with real code.

Minimum coverage required (enforced by the template structure):

| Test case | Default behavior | What it catches |
|-----------|-----------------|----------------|
| `skill.md reachable + contains scenario_id` | Passes immediately | Broken skillmd render, wrong route |
| `agent.json reachable + lists endpoints` | Passes immediately | Scaffold wiring failure |
| `register is idempotent` | Passes immediately | Double-registration bug |
| `complete agent journey` | **Fails until FILL done** | Full lifecycle correctness |
| `duplicate action returns 409/422` | **Fails until FILL done** | Missing idempotency guard |

If the scenario has a forfeit/leave endpoint, the forfeit test case stub must also be filled.

`npm test` green with unfilled FILL zones = **impossible** — `expect.fail()` prevents it.
