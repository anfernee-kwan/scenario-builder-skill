...
| **relationship** | `cross_cutting` includes `"relationship"` | `relationship/schema.partial.ts` — drizzle `relationships` table (ordered agent pair, score −100~+100, affinity label); `src/blocks/relationship.ts` — `getRelationship`, `updateRelationship`, `listRelationships` helpers | Schema partial merged into `src/db/schema.ts`; Fill's interaction routes (react/comment/DM) call `updateRelationship()` |
| **memory** | `cross_cutting` includes `"memory"` | `memory/schema.partial.ts` — drizzle `agent_memories` table (agentId, kind, content, importance 1–10); `src/blocks/memory.ts` — `storeMemory`, `recallMemories` helpers | Schema partial merged into `src/db/schema.ts`; engine loop calls `storeMemory()` after significant events; Fill's LLM prompt includes recalled memories |
| **notification** | `cross_cutting` includes `"notification"` | `notification/schema.partial.ts` — drizzle `user_agent_bindings` table (agentId ↔ email) + `notifications` table; `src/blocks/notification.ts` — `bindAgentEmail`, `queueNotification`, `sendNotification` (env-var 多 provider: Resend / mock log); `.env.example` 追加 `RESEND_API_KEY`/`EMAIL_FROM` 注释 | Schema partial merged into `src/db/schema.ts`; Fill's bind endpoint calls `bindAgentEmail()`; engine loop calls `queueNotification()` + `sendNotification()` for significant events |
